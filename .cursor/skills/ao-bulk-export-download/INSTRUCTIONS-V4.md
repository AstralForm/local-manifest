...
```text
Ready. Please paste your raw text dump. I'll generate a downloadable Bash script named after the entity with valid download commands, interactive prompts for Case / EntityName / CompanyName / User_mail / UserID / CID / Senior_Lead_Reviewer, Jira AOPS creation (token + cloud ID embedded — no prompts), one optional Slack workflow trigger including Jira_link, and macOS run commands.
```
...
3. Creates one Jira `AOPS` Task after downloads finish using embedded scoped token + cloud ID (**do not prompt** for them).
4. Sets `Jira_link` to the created issue browse URL, or `n/a` if Jira creation fails.
5. Asks whether to send the Slack notification.
6. If confirmed, sends exactly one Slack Workflow webhook payload using the required variable names (webhook URL embedded — **do not prompt** for it).
7. Prints a clear summary and macOS run commands.
...
- Jira creation after downloads and before Slack (no cloud-id/token prompts)
- Slack confirmation after Jira
...
Create one Jira issue after downloads complete. The Jira issue must be created by the generated script, not by a Slack workflow button.
...
Do not create duplicate Jira issues. Create exactly one issue per script run.
...
After Jira handling, ask exactly:
...
3. A short confirmation covering:
   ...
   - required runtime prompts;
   - Jira AOPS creation;
   - optional Slack workflow payload with `Jira_link`.
